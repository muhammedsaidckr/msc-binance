...
namespace Mscakir\MscBinance\Helpers;
...
class SecureString
{
     ...
     */
    public function timestampExpiry(int $value = 0) : int
    {
        if ($value) {
     ...
     * @throws Exception
     */
    public function encode(mixed $data, array $spec, bool $throw = false) : string|null
    {
     ...
     * @throws Exception
     */
    public function decode(string $str, array $spec, bool $throw = false) : string|null
    {
        $decodeFirst = false;
     ...
     */
    public static function generateRandomKey(int $len = 32): string
    {
        $bytes = ceil($len * 3 / 4);
     ...
     * @throws Exception
     */
    protected function encrypt(string $str, string $method, string $password, string $iv = '', bool $throw = false) : string
    {
        if ($iv === '') {
     ...
     * @throws Exception
     */
    protected function decrypt(string $str, string $method, string $password, bool $throw = false) : string|null
    {
     ...
     * @return array|null|bool Returns an array in the form [encode, decode, method, encodeFirst].
     * Returns null on error.
     * @throws Exception
     */
    protected function supportedInfo(string $name, bool $throw = false) : array|null|bool
    {
        switch ($name) {
            case 'aes128':
            case 'aes256':
     ...
     */
    protected function hmac(string $str, string $method, string $password, int $timestamp = 0, bool $throw = false) : string
    {
        if ($timestamp === 0) {
            $timestamp = time();
        }
     ...
     * @return bool|null Returns the string without the signing information or null on error.
     * @throws Exception
     */
    protected function verifyHmac(string $str, string $method, string $password, bool $throw = false) : bool|null
    {
     ...
     * @throws Exception
     */
    protected function verifyTimestamp(string $timestamp, bool $throw = false) : bool
    {
     ...
     */
    protected function popString(string &$str) : string|null
    {
        if ($str === '') {
            return null;
        }
     ...
     */
    protected function pushString(string &$str, string|array $item) : void
    {
        if ($str) {
            $str .= static::SEP;
        }
     ...
     * @return bool|null Returns false if {@link $throw} is false.
     * @throws Exception Throws an exception with {@link $message} and {@link $code}.
     */
    protected function exception(bool $throw, string $message, int $code) : bool|null
    {
        if ($throw) {
     ...
     */
    public function twiddle(string $string, int $index, string $value, bool $encode = false) : string
    {
